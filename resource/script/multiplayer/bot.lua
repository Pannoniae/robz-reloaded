...
function GetFlagPriority(flag)
	if		IsCapturedFlag(flag)then return FlagPriority.Captured / isAirborne
	elseif	IsEnemyFlag(flag)	then return FlagPriority.Enemy * isAirborne
	else 							 return FlagPriority.Neutral / isAirborne
	end
end
...
function GetFlagToCapture(flagPoints, getPriority)
	...
	for i, flag in pairs(flagPoints) do
		-- flag side >
		local side = BotApi.Instance.team
		if ((string.find(flag.name,side .. "_") == nil) and (string.find(flag.name,"f") == nil))
		or (string.find(flag.name,"f") ~= nil)
		then
		-- flag side <
...
function GetNextUnitToSpawn(purchase)
	...
	purchase:moveNext()
	if unit.class == UnitClass.Airborne then
      isAirborne = 2
    end
    return unit
end
...
function GetUnitToSpawn(units)
	...
	for i, flag in pairs(BotApi.Scene.Flags) do
		isfl = 0
		if (string.find(BotApi.Instance.team,"a") ~= nil) then
			if (string.find(flag.name,"f21") ~= nil) then
				isfl = 1
			end
			if (string.find(flag.name,"f22") ~= nil) then
				isfl = 1
			end
		end
	...
	return GetRandomItem(unitsToSpawn, function(t)
		...
		if t.class == UnitClass.Airborne then
			return 0
		end
		if t.class == UnitClass.Howitzrer then
			return 0
		end
		if t.class == UnitClass.Rare and isRare == 0 then
			isRare = 1
		end
		if t.class == UnitClass.Rare and isRare == 1 then
			return 0
		end
...
function OnGameSpawn(args)
	isAirborne = 1
	isRare = 0
	--[[ timers for combat mode >
	for i, flag in pairs(BotApi.Scene.Flags) do
		local side = BotApi.Instance.team
		if (string.find(flag.name,side .. "_") ~= nil)  then
			OrderRotationPeriod = OrderRotationPeriod * 1.2
		end
